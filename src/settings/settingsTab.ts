import type WritrGoalsPlugin from "src/plugin/main";
import {
	App,
	Setting,
	PluginSettingTab,
...
import { WRITR_GOALS_VIEW_SIDE } from "./settingsData";

export class WritrGoalsSettingsTab extends PluginSettingTab {
	...
	display(): void {
		...
		containerEl.empty();
		containerEl.createEl("h1", { text: "Writr Goals" });
		...
		new Setting(containerEl)
			.setName("Position status board view")
			.setDesc(
				"Choose which side panel you'd like the status board view to bind (right or left)"
			)
			.addDropdown((dropdown) => {
				let directions: Record<string, string> = {};
				WRITR_GOALS_VIEW_SIDE.map(
					(direction) => (directions[direction] = direction)
				);
				dropdown.addOptions(directions);
				dropdown
					.setValue(this.plugin.settings.writrGoalsViewPosition)
					.onChange((viewPosition) => {
						this.plugin.settings.writrGoalsViewPosition =
							viewPosition;
						this.plugin.saveSettings();
					});
			});
		const msDonationDiv = containerEl.createEl("div", {
			cls: "wgDonationSection",
		});
